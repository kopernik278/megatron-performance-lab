#!/usr/bin/env bash
# Phase 8.3: interleaved 1F1B + VPP=2 + overlap_p2p_comm A/B on one 2x A40 pod.
# Do not combine TP and PP. Do not disable NCCL P2P. Do not change pipeline dtype.
set -euo pipefail
...
BRANCH="${PHASE83_BRANCH:-cursor/phase83-interleaved-pp-overlap-3b5c}"
POD_ID="${1:?pod id required}"
PRICE="${2:-0.88}"
PUBLIC_IP="${PHASE83_PUBLIC_IP:-}"
DATA_CENTER="${PHASE83_DATA_CENTER:-}"
...
ABORT_MARKER=/tmp/phase83_abort_reason.txt
WORK=results/phase83_work
PROF=profiles/phase83_work
...
if [[ "${NCCL_P2P_DISABLE:-0}" == "1" ]]; then
  echo "NCCL_P2P_DISABLE must not be set for Phase 8.3" >&2
  exit 1
fi
...
topology = {}
path = Path("results/phase83_work/topology.json")
if path.exists():
    try:
...
Path("results/phase8_interleaved_pp_overlap.json").write_text(
    json.dumps(
        {
            "status": "aborted",
...
Path("docs/experiments/phase8_interleaved_pp_overlap.md").write_text(
    f"# Phase 8.3 aborted\n\n{reason}\n",
    encoding="utf-8",
)
PY
...
case "${driver}" in
  570.*|580.*) ;;
  *)
    abort "Rejecting driver ${driver}; Phase 8.3 requires 570.x or 580.x with the cu128 image"
    ;;
esac
...
import json
from pathlib import Path
payload = json.loads(Path("results/phase83_work/topology.json").read_text())
...
Path("results/phase83_work/topology.json").write_text(json.dumps(payload, indent=2) + "\n")
print(f"PHASE83_TOPOLOGY_OK path={path}")
PY

echo "PHASE83_CORRECTNESS_SMOKE_START"
set +e
RUN_TIMEOUT_SEC=180 run_dist 2 \
  scripts/phase8_interleaved_pp_run.py \
  --run-label phase83_b_smoke \
  --smoke-only \
...
echo "PHASE83_CORRECTNESS_SMOKE_OK"

echo "PHASE83_VARIANT_A_START"
RUN_TIMEOUT_SEC=900 run_dist 2 \
  ...
  --run-label phase83_a_noninterleaved \
  --output-json "${WORK}/variant_a.json"

RUN_TIMEOUT_SEC=900 run_nsys 2 scripts/phase8_pp_run.py \
  phase83_a_profile "${PROF}/variant_a" \
  --tensor-parallel-size 1 \
  --pipeline-parallel-size 2 \
  --num-microbatches 4 --micro-batch-size 2 --global-batch-size 8 \
...
echo "PHASE83_VARIANT_B_START"
RUN_TIMEOUT_SEC=900 run_dist 2 \
  scripts/phase8_interleaved_pp_run.py \
  --num-microbatches 4 --micro-batch-size 2 --global-batch-size 8 \
  --smoke-iterations 3 --warmup-iterations 5 --measured-iterations 20 \
  --run-label phase83_b_interleaved_overlap \
  --output-json "${WORK}/variant_b.json"

RUN_TIMEOUT_SEC=900 run_nsys 2 scripts/phase8_interleaved_pp_run.py \
  phase83_b_profile "${PROF}/variant_b" \
  --num-microbatches 4 --micro-batch-size 2 --global-batch-size 8 \
...
import json
from pathlib import Path
a = json.loads(Path("results/phase83_work/variant_a.json").read_text())
b = json.loads(Path("results/phase83_work/variant_b.json").read_text())
print((b["tokens_per_second"] - a["tokens_per_second"]) / a["tokens_per_second"] * 100.0)
PY
)"
echo "PHASE83_FAST_GAIN_PERCENT=${gain}"
need_formal="$("${PYTHON}" -c "print('yes' if float('${gain}') >= 3.0 else 'no')")"
if [[ "${need_formal}" == "yes" ]]; then
  echo "PHASE83_FORMAL_B_START"
  RUN_TIMEOUT_SEC=2400 run_dist 2 \
    scripts/phase8_interleaved_pp_run.py \
    --num-microbatches 4 --micro-batch-size 2 --global-batch-size 8 \
    --smoke-iterations 3 --warmup-iterations 20 --measured-iterations 100 \
    --run-label phase83_b_formal \
    --output-json "${WORK}/variant_b_formal.json"
  FORMAL_ARGS=(--formal-b "${WORK}/variant_b_formal.json")
fi

"${PYTHON}" scripts/phase8_analyze_interleaved_pp.py \
  ...
  --pod-id "${POD_ID}" \
  --price-per-hour-usd "${PRICE}" \
  --output results/phase8_interleaved_pp_overlap.json \
  --markdown docs/experiments/phase8_interleaved_pp_overlap.md \
  "${FORMAL_ARGS[@]+"${FORMAL_ARGS[@]}"}"

echo "PHASE83_POD_RUN_COMPLETE"
